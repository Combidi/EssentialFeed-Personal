...
class CodableFeedStoreTests: XCTestCase {
    ...
    func test_retrieveAfterInsertingToEmptyCache_deliversInsertedValues() {
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            XCTAssertNil(insertionError, "Expecteed feed to be inserted successfully")
            exp.fulfill()
        }
        wait(for: [exp], timeout: 1.0)
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    ...
                    default:
                        XCTFail("Expected retrieving twice from non empty cache to deliver same found result with feed \(feed) and \(timestamp), got \(firstResult) and \(secondResult)")
                    }
    ...
    private func expect(_ sut: CodableFeedStore, toReceive expectedResult: RetrieveCachedFeedResult, file: StaticString = #filePath, line: UInt = #line) {
        let exp = expectation(description: "Wait for cache retrieval")
